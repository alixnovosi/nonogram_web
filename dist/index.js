...
class Game extends React.Component {
    constructor(props) {
        ...
        this.state = Object.assign({}, this.state, this.decodeSquares());
        // generate hints,
        // and then force blank rows based on those hints.
        this.state = Object.assign({}, this.state, this.genHints());
        this.state = Object.assign({}, this.state, this.setBlockedSections());
        // finally,
        // reformat display hints.
        this.state = Object.assign({}, this.state, this.genDisplayHints());
    }
    decodeSquares() {
        let base64Blob = this.state.code.replace(/_/g, "/").replace(/-/g, "+");
        let zipped;
        try {
        ...
        let boardDict = JSON.parse(new TextDecoder("utf-8")
        ...
        for (let h = hexString.length - 1; h >= 0; h--) {
            ...
            for (var j = 3; j >= 0; j--) {
                ...
                let c = i % width;
                squares[r][c] =
                    <Square key={i} filled={filled} displayValue={defaultDisplayValue} blocked={false} onClick={() => this.handleClick(r, c)} solved={this.state.solved}/>;
                i--;
                if (i < 0) {
                    break;
                }
        ...
        for (let [r, row] of squares.entries()) {
            for (let [c, square] of row.entries()) {
                ...
                if (this.state.leftHints[r][0] === 0 ||
                    ...
                    squares[r][c] = React.cloneElement(square, {
                        blocked: true,
    ...
    genDisplayHints() {
        ...
        let displayTopHints = [...Array(this.state.maxTopHintSize).keys()]
            .map(() => Array(this.state.maxLeftHintSize).fill(null));
    ...
    handleClick(r, c) {
        ...
        let newDisplayValue = SQUARE_VALUES_LIST[(SQUARE_VALUES_LIST.indexOf(oldSquare.props.displayValue) + 1)
            % SQUARE_VALUES_LIST.length];
        squares[r][c] = React.cloneElement(oldSquare, {
    ...
    render() {
        const reset = <button onClick={() => this.reset()}>
                Reset
            </button>;
        const validate = <button onClick={() => this.validate()}>
                Validate
            </button>;
        let gameInfo = null;
